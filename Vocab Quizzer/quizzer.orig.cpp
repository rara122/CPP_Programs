...
int main(){
  ...
  if(option == 1){
    ...
    while(fileExists(filenamecheck.c_str()) != 0){
      ...
      cin >> number;
      while (checkInput(number)==-1){
        cout << "Bad Input: Reenter Number of items in the list: ";
        cin >> number;
        }
      j = atoi(number.c_str());
      listCreator(i, j);
    ...
    if(i == 1)
      ...
      cin >> number;
	  while (checkInput(number)==-1){
        cout << "Bad Input: Reenter File to view: ";
        cin >> number;
        }
      i = atoi(number.c_str());
	  string filenamecheck;
      ...
      filenamecheck = sst.str();
      while(fileExists(filenamecheck.c_str()) == 0){
        cout << "Does Not Exist Reenter File to view: ";
        cin >> number;
        while (checkInput(number)==-1){
          cout << "Bad Input: Reenter File to view: ";
        cin >> number;
...
if(!fin){
	cout << "Statistics.txt failed to open.\nCheck to make sure that it exists.\n\n";
	return -1;
}
string word;
string day;
